...
namespace TrainworksModdingTools.Enums
{
        ...
        where TEnum : Enum
    {
        ...
        public ExtendedEnum(string Name, int ID)
        {
            ...
            {
                TrainworksModdingTools.Trainworks.Log(BepInEx.Logging.LogLevel.Warning, $"Name: {this.Name} Conflict in domain, {typeof(TExtendedEnum).Name}");
            }
            if (IntToExtendedEnumMap.ContainsKey(this.ID))
            {
                TrainworksModdingTools.Trainworks.Log(BepInEx.Logging.LogLevel.Warning, $"ID#{this.ID} Conflict between {Name} and {IntToExtendedEnumMap[this.ID].Name} in domain, {typeof(TExtendedEnum).Name}");
            }
            if (ReservedIDs.Contains(this.ID))
            {
                TrainworksModdingTools.Trainworks.Log(BepInEx.Logging.LogLevel.Warning, $"ID#{this.ID} is Reserved and can't be set for {Name}");
            }
        ...
        /// <summary>
        /// Returns a generated variant of TEnum that can be used for framework functions
        /// </summary>
