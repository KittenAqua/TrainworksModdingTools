    ...
    public class BundleManager
    {
        ...
        public static void RegisterBundle(string assetGUID, BundleAssetLoadingInfo bundleInfo)
        {
            ...
            if (!LoadedAssetBundles.ContainsKey(path))
            {
                ...
                {
                    API.Log(BepInEx.Logging.LogLevel.Warning, "Custom asset bundle failed to load from path: " + path);
                }
            }
        ...
        public static UnityEngine.Object LoadAssetFromBundle(BundleAssetLoadingInfo info, string assetName)
        {
            if (LoadedAssetBundles.ContainsKey(info.FullPath))
            {
                ...
                if (asset == null)
                {
                    API.Log(BepInEx.Logging.LogLevel.Warning, "Custom asset: " + assetName + " failed to load from bundle: " + info.FullPath);
                }
                ApplyImportSettings(info, ref asset);
                return asset;
            }
            API.Log(BepInEx.Logging.LogLevel.Warning, "Attempting to load asset from non-existent bundle: " + info.FullPath);
            return null;
        }
